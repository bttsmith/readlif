...
class LifImage:
    ...
    def __repr__(self):
        return repr(str(self.name)+': a LifImageArr object with dimensions ' + str(self.dims))
    ...
    def _get_item(self, n):
        ...
        if self.bit_depth[0] == 8:
            return Image.frombytes("L",
                                   ...
                                   data)
        elif self.bit_depth[0] == 12:
            return Image.frombytes("I;16",
                                   (self.dims_n[self.display_dims[0]]*16,
                                    self.dims_n[self.display_dims[1]]*16),
                                   data)
...
class LifFile:
            ...
            if is_image:
                ...
                for d in dims:
                    ...
                    try:
                        ...
                        if dim_n == 4:
                            scale_dict[s] = ((int(dims_dict[dim_n]))
                                                 / float(len_n))
    ...
    def get_image(self, img_n=0):
        """
        Specify the image number, and this returns a LifImageArr object
        of that image.
        ...
        Returns:
            LifImageArr object with specified image
        """
        img_n = int(img_n)
        ...
        return LifImageArr(image_info, offsets, self.filename)

    def get_iter_image(self, img_n=0):
        """
        Returns an iterator of LifImageArr objects in the lif file.

        Args:
            img_n (int): Image to start iteration at

        Returns:
            Iterator of LifImageArr objects.
        """
        img_n = int(img_n)
...
class LifImageArr:
    ...
    def _get_item(self, n):
        ...
        img_shape = (self.dims_n[self.display_dims[1]],
                     self.dims_n[self.display_dims[0]]
                     )
        ...
        # However, it is safer to let the lif file tell us the resolution
        if self.bit_depth[0] == 8:
            return np.frombuffer(data, dtype = 'u2').reshape(img_shape)
        elif self.bit_depth[0] == 12:
            return np.frombuffer(data, dtype = 'u2').reshape(img_shape)*16
        elif self.bit_depth[0] <= 16:
            return np.frombuffer(data, dtype = 'u2').reshape(img_shape)
        else:
            raise ValueError("Unknown bit-depth, please submit a bug report"
    ...
    def get_plane(self, display_dims=None, c=0, requested_dims=None):
        ...
        # However, it is safer to let the lif file tell us the resolution
        if self.bit_depth[0] == 8:
            return np.frombuffer(data, dtype = 'u1').reshape(img_shape)
        elif self.bit_depth[0] == 12:
            return np.frombuffer(data, dtype = 'u2').reshape(img_shape)*16
        elif self.bit_depth[0] <= 16:
            return np.frombuffer(data, dtype = 'u2').reshape(img_shape)
